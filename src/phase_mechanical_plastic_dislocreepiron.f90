...
!--------------------------------------------------------------------------------------------------
!> @author Ethan Edmunds, The University of Sheffield
!> @brief Dislocation-density based crystal plasticity model for creep in bcc metals, especially Iron
!--------------------------------------------------------------------------------------------------
submodule(phase:plastic) dislocreepiron
  ...
  type :: tDislocreepironState
    real(pREAL), dimension(:,:), pointer :: &
      rho_mob, &
      rho_dip, &
      gamma_sl
  end type tDislocreepironState

  type :: tDislocreepironDependentState
    real(pREAL), dimension(:,:), allocatable :: &
      Lambda_sl, &
      tau_pass
  end type tDislocreepironDependentState

!--------------------------------------------------------------------------------------------------
! containers for parameters and state
  type(tParameters),                  allocatable, dimension(:) :: param
  type(tIndexDotState),               allocatable, dimension(:) :: indexDotState
  type(tDisloCreepIronState),          allocatable, dimension(:) :: state
  type(tDisloCreepIronDependentState), allocatable, dimension(:) :: dependentState
...
module function plastic_dislocreepiron_init() result(myPlasticity)
  ...
  myPlasticity = plastic_active('dislocreepiron')
  if (count(myPlasticity) == 0) return

  print'(/,1x,a)', '<<<+-  phase:mechanical:plastic:dislocreepiron init  -+>>>'
...
!--------------------------------------------------------------------------------------------------
pure module subroutine dislocreepiron_LpAndItsTangent(Lp,dLp_dMp, &
                                                     Mp,ph,en)
...
!--------------------------------------------------------------------------------------------------
module subroutine plastic_dislocreepiron_result(ph,group)
...
end subroutine plastic_dislocreepiron_result
